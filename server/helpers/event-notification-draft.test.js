...
test('stripHtml converts common editor HTML to readable text', () => {
  assert.equal(stripHtml('<p>Hello&nbsp;<strong>team</strong></p><p>Use A&amp;B</p>'), 'Hello team\nUse A&B');
});
...
test('splitRecipients deduplicates email addresses and reports missing emails', () => {
  ...
  assert.deepEqual(
    result.missingEmails.map((r) => r.fullName),
    ['No Email']
  );
});
...
test('resolveEventNotificationDraft resolves workflow participants and missing emails', async () => {
  const users = {
    ...
    4: {
      id: 4,
      full_name: 'Supervisor Duplicate',
      email: 'ds@example.com',
      role: 'SUPERVISOR',
      department_name: 'Home',
      department_id: 20,
  ...
  assert.deepEqual(draft.recipients.map((r) => r.email).sort(), [
    'collab@example.com',
    'ds@example.com',
    'sc@example.com',
  ]);
  assert.deepEqual(
    draft.missingEmails.map((r) => r.fullName),
    ['Missing Email']
  );
});
